...
function SearchPage() {
  ...
  // READ — localStorage se history load karo
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    debounceTimer.current = setTimeout(async () => {
      ...
      try {
        ...
        // CREATE — search history mein add karo
        const saved = JSON.parse(localStorage.getItem('searchHistory') || '[]')
  ...
  // DELETE — history se item hatao
  const handleDelete = (index) => {
    const updated = history.filter((_, i) => i !== index)
    saveHistory(updated)
  }

  // UPDATE — history item edit karo
  ...
  return (
    <div className={`min-h-screen px-6 py-12 ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'}`}>
      ...
      {/* Result */}
      {result && (
        <div className={`max-w-xl mx-auto rounded-xl p-6 mb-8 ${darkMode ? 'bg-gray-800' : 'bg-white shadow-md'}`}>
          ...
          <div className={`space-y-2 text-sm ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
            <p>🌍 <span className="font-medium">Country:</span> {result.countryName || result.countryCode || 'N/A'}</p>
            <p>🏢 <span className="font-medium">ISP:</span> {result.isp}</p>
            <p>⚠️ <span className="font-medium">Abuse Score:</span> {result.abuseConfidenceScore}%</p>
            <p>📊 <span className="font-medium">Total Reports:</span> {result.totalReports}</p>
            <p>🕒 <span className="font-medium">Last Reported:</span> {result.lastReportedAt || 'Never'}</p>
          </div>
        </div>
      )}
      ...
      {/* Search History — CRUD */}
      {history.length > 0 && (
        <div className={`max-w-xl mx-auto rounded-xl p-6 ${darkMode ? 'bg-gray-800' : 'bg-white shadow-md'}`}>
          <h2 className="text-green-400 text-lg font-semibold mb-4">🕘 Search History</h2>
          <div className="space-y-2">
            {history.map((item, index) => (
              <div key={index} className={`flex items-center justify-between rounded-lg px-3 py-2 ${darkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
                {editIndex === index ? (
                  <>
                    <input
                      value={editValue}
